...
public:
	...
	/*void ver() const {
		if (raiz == nullptr) cout << "nullptr raiz" << endl;
		else {
			cout << raiz->elem << endl;		cout << raiz->numNodosIz << endl;

			if (raiz->iz == nullptr) cout << "nullptr raiz->iz" << endl;

			else {
				cout << raiz->iz->elem << endl; cout << raiz->iz->numNodosIz << endl;
			}
			if (raiz->dr == nullptr) cout << "nullptr raiz->dr" << endl;

			else {
				cout << raiz->dr->elem << endl; cout << raiz->dr->numNodosIz << endl;




				if (raiz->dr->dr == nullptr)
					cout << "nullptr raiz->dr->dr" << endl;

				else {
					cout << raiz->dr->dr->elem << endl; cout << raiz->dr->dr->numNodosIz << endl;
				}
				}
	...
	T const& kesimo(int k) const {
		if (k > this->size()) throw ENumElemsMenorQueK();

		return kesimoAux(raiz, k);


	}
	static int numNodosIzVer(Link a) {
		if (a == nullptr) return 0;
		else return a->numNodosIz;
	}
